  ...
  class Reudy
    ...
    def replaceWords(base, new_words, toForce)
      ...
      new_words.shuffle.each do |new_word|
        ...
        """
        old_word_str = parts[rand(wordCt)]
        0.upto(wordCt+1) do |j|
          if parts[j] == "" || old_word_str == ""
            next
          end
          #puts "before: #{parts[j]}"
          #puts "#{parts[j] == old_word_str && !already_used.include?(parts[j]) && !already_used.include?(old_word_str)}, #{new_word_str}, #{parts[j]} == #{old_word_str}"
          #puts "already_used: #{already_used}"
          parts[j] = new_word_str if parts[j] == old_word_str && !already_used.include?(parts[j]) && !already_used.include?(old_word_str)
          already_used.push(parts[j]) if !already_used.include?(parts[j])
          already_used.push(old_word_str) if !already_used.include?(old_word_str)
          #puts "after: #{parts[j]}"
        end
        break if rand < 0.5
      end
    ...
    def onDiscMove(channelid, channelname, username)
      @log.addMsg(username, "!command disc_move #{channelid} #{channelname}")
    end
